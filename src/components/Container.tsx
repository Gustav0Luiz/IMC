        ...
        <div className="flex max-w-xl lg:max-w-5xl m-auto gap-3 lg:gap-20 font-sans px-3 flex-col lg:flex-row">
                    ...
                    <div className="grid flex-1 grid-cols-2 gap-2 lg:gap-5">
                        {levels.map((item) =>( 
                            <div className="flex-1 flex font-medium  text-white rounded-xl gap-5 flex-col p-5 text-xl items-center justify-center max-h-40 lg:max-h-full" style={{backgroundColor:item.color}}>   
                                ...
                                <div className="w-10 h-10 lg:w-14 lg:h-14 rounded-full lg:bg-black/20 items-center flex justify-center ">{item.icon === 'up' ? <Image alt={'up'} width={30} src={upImage}/> : <Image alt={'down'} width={30} src={downImage}/>  }</div>
            ...
            {showItem &&
                ...
                <div className="grid flex-1 grid-cols-1 gap-5">
                    ...
                    <h1 className="text-center text-2xl lg:text-4xl">{showItem.title}</h1>   
                    ...
                    <div className="text-lg lg:text-xl">{`Seu IMC é de ${showItem.yourImc?.toFixed(2)} Kg/m²!`}</div>
                    <div className="text-base font-normal ">{`IMC está entre ${showItem.imc[0]} e ${showItem.imc[1]}`}</div>
                    <button onClick={handleBackBtn} className="w-60 px-2 py-1 lg:px-4 lg:py-2 rounded-lg text-white bg-cyan-700/90 text-lg hover:opacity-80 transition duration-500 ease-in-out">Voltar</button>
                </div>
