...
export const Container = () => {
    ...
    return(
        <div className="flex max-w-xl lg:max-w-5xl m-auto gap-8 lg:gap-20 font-sans px-3 flex-col lg:flex-row">
            <div className="flex-1 ">
                <h1 className="font-bold text-3xl lg:text-4xl mb-9 text-center lg:text-left">Calcule o seu IMC.</h1>
                <p className=" text-justify text-base lg:text-lg mb-10 leading-6">IMC é a sigla para Índicie de Massa Corpórea, parametro adotado pela OMS para calcular o peso ideal de cada pessoa.</p>
                <input className="w-full rounded-md border-b-2 border-solid border-b-gray-800/30 px-4 py-2 mb-4 text-base outline-none" type="number" value={heightField > 0 ? heightField : ''} onChange={e => setHeightField(parseFloat(e.target.value))} placeholder="Digite a sua Altura. Ex (1.75)" disabled={showItem ? true : false}/>
                <input className="w-full rounded-md border-b-2 border-solid border-b-gray-800/30 px-4 py-2 mb-4 text-base outline-none" type="number" value={weightField > 0 ? weightField : ''} onChange={e => setWeightField(parseFloat(e.target.value))} placeholder="Digite a seu Peso. Ex (70)" disabled={showItem ? true : false} />
